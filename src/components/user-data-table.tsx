...
export function UserDataTable({ sectionTitle, fields, userData }: UserDataTableProps) {
        ...
        <Table className="w-full text-sm text-gray-700">
          ...
          <TableBody>
            {Object.entries(fields).map(([fieldKey, label]) => {
              ...
              const isDateField = ["dateOfBirth", "disbursementDate"].includes(fieldKey)

              let display: React.ReactNode

              if (fieldKey === "approvedCreditValue") {
                display = formatMoney(rawValue as string)
              }
              else if (Array.isArray(rawValue)) {
                const translatedItems = rawValue.map((item) =>
                  valueTranslations[fieldKey]?.[item] || item
                )
                display = (
                ...
                display = <span className="text-red-600 font-medium">⚠️ —</span>
              }
