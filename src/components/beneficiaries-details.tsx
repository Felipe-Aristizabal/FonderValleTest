...
export function BeneficiariesDetails({
  ...
  if (!userData) {
    return (
      <p className="text-center py-4 text-red-500">Usuario no encontrado.</p>
    );
  }
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-10 space-y-10">
      <div className="text-center border-b pb-4">
        <h2 className="text-3xl font-bold">Detalle de {userData.fullName}</h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-6 gap-6">
        <div className="md:col-span-4 bg-white rounded-lg shadow-md p-6 space-y-6">
          {Object.entries(sectionedFieldLabels).map(
            ([sectionTitle, fieldsMap]) => (
        ...
        <div className="bg-white rounded-lg md:col-span-2 shadow-md p-6 space-y-8">
          <BeneficiariesVisitsTable userId={userId} visits={visits} />
        </div>
      </div>
